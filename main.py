...
try:
    ...
    from app.modules.search import NQueensProblem, KnightsTourProblem, TowerOfHanoiProblem, GraphColoringProblem
    ...
    from app.gui.components import (render_interactive_queens_board, board_to_text, check_queens_validity,
                                     ...
                                     render_interactive_graph_coloring, check_graph_coloring_validity)
...
with st.sidebar:
    ...
    problem_type = st.radio(
        ...
        ("Jocuri (Echilibru Nash)", "Căutare (N-Queens)", "Căutare (Turul Calului)", "Căutare (Turnurile Hanoi)", "CSP (Graph Coloring)")
    )
...
if st.session_state.problem_type != problem_type:
    ...
    elif problem_type == "Căutare (Turnurile Hanoi)":
        st.session_state.game = TowerOfHanoiProblem()
    elif problem_type == "CSP (Graph Coloring)":
...
with col_left:
    ...
    if st.session_state.matrix:
        ...
        if problem_type == "Jocuri (Echilibru Nash)":
            ...
            board_size = len(st.session_state.matrix)
            pdf_req = f"Se da graful cu {board_size} noduri definit de matricea de adiacenta de mai jos (1 = muchie, 0 = fara muchie). Colorati nodurile astfel incat nicio pereche de noduri adiacente sa nu aiba aceeasi culoare."
...
with col_right:
    ...
    if st.session_state.matrix:
        ...
        # --- 5. GRAPH COLORING ---
            ...
            adj_matrix = st.session_state.matrix
            st.markdown("### Cerință:")
            st.write(f"Identifică o colorare validă pentru graful de **{len(adj_matrix)} noduri** definit mai jos.")
            
            with st.expander("🔢 Vezi Matricea de Adiacență"):
                df_adj = pd.DataFrame(
                    adj_matrix,
                    index=[f"Nod {i}" for i in range(len(adj_matrix))],
                    columns=[f"Nod {i}" for i in range(len(adj_matrix))]
                )
                st.table(df_adj)

            user_colors = render_interactive_graph_coloring(adj_matrix, key_prefix="gc_user")
        ...
        if problem_type == "Jocuri (Echilibru Nash)":
            ...
            if st.button("✅ Verifică Răspunsul", type="primary"):
                ...
                if not is_valid:
                    ...
                    with st.expander("📊 Vezi analiza detaliată AI"):
                        ...
                        st.write(f"- 📍 Acoperire tabla: {metrics['coverage']*100:.1f}%")
                        st.write(f"- ✅ Mișcări valide: {metrics['validity']*100:.1f}%")
                        st.write(f"- 🎯 Similaritate cu soluția: {metrics['similarity']*100:.1f}%")
                else:
                    st.success(validity_msg)
                    st.markdown(f"### Scor Final: **100.00%**")
                ...
                with st.expander("🔍 Vezi Soluția (Gold Standard)"):
                    st.info(st.session_state.correct_expl)
                    # Simple visualization for solution
                    solution_display = []
                    for row in solution_board:
                        solution_display.append([str(cell) if cell >= 0 else "·" for cell in row])
                    st.dataframe(pd.DataFrame(solution_display), use_container_width=True)
            ...
            if st.button("✅ Verifică Răspunsul", type="primary"):
                # Get the optimal chromatic number calculated during generation
                min_colors = st.session_state.game.min_colors
                
                # Verify with all 3 parameters
                is_valid, msg, feedback, score = check_graph_coloring_validity(
                    adj_matrix, 
                    user_colors, 
                    min_colors
                )
                ...
                if score == 100:
                    st.success(msg)
                    st.markdown(f"### Scor Final: **{score:.2f}%**")
                elif is_valid:
                    st.warning(msg)
                    st.markdown(f"### Scor Final: **{score:.2f}%**")
                else:
                ...
                if feedback:
                    st.markdown("### 📝 Feedback Detaliat:")
                    for item in feedback:
                        if "conflict" in item.lower() or "conectate" in item.lower():
                            st.warning(item)
                        else:
                            st.info(item)
                
                with st.expander("🔍 Vezi Soluția Algoritmică"):
                    st.info(st.session_state.correct_expl)
